...
<div class="dashboard-wrapper">
    ...
    <div class="main-content">
        <div class="topbar">
            <div>
                <h3 style="color: var(--dark); font-size: 1.8rem; margin-bottom: 0.3rem;">Tambah Data Penduduk</h3>
                <p style="color: #64748b; font-size: 0.95rem;">Tambahkan data penduduk baru ke dalam sistem</p>
            </div>
        ...
        <form action="<?= base_url('/penduduk/simpan') ?>" method="POST" id="formTambahPenduduk">
            <?= csrf_field() ?>
            
            <div class="card">
                <h4 style="color: var(--dark); margin-bottom: 1.5rem; display: flex; align-items: center; gap: 0.5rem;">
                    <i class="ri-user-add-line" style="color: var(--primary);"></i> Data Identitas
                </h4>
                ...
                <div style="display: grid; gap: 1.5rem;">
                    ...
                    <div>
                        ...
                        <textarea id="alamat" name="alamat" rows="3" required
                                  ...
                                  placeholder="Masukkan alamat lengkap (RT/RW, Desa, Kecamatan)"><?= old('alamat') ?></textarea>
                    </div>
                    ...
                    <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 1rem;">
                        <div>
                            ...
                            <select id="status" name="status" required
                                ...
                                <option value="Kepala Keluarga" <?= old('status') == 'Kepala Keluarga' ? 'selected' : '' ?>>Kepala Keluarga</option>
                                <option value="Istri" <?= old('status') == 'Istri' ? 'selected' : '' ?>>Istri</option>
                                <option value="Anak" <?= old('status') == 'Anak' ? 'selected' : '' ?>>Anak</option>
                                <option value="Menantu" <?= old('status') == 'Menantu' ? 'selected' : '' ?>>Menantu</option>
                                <option value="Cucu" <?= old('status') == 'Cucu' ? 'selected' : '' ?>>Cucu</option>
                                <option value="Orang Tua" <?= old('status') == 'Orang Tua' ? 'selected' : '' ?>>Orang Tua</option>
                                <option value="Mertua" <?= old('status') == 'Mertua' ? 'selected' : '' ?>>Mertua</option>
                                <option value="Famili Lain" <?= old('status') == 'Famili Lain' ? 'selected' : '' ?>>Famili Lain</option>
                                <option value="Pembantu" <?= old('status') == 'Pembantu' ? 'selected' : '' ?>>Pembantu</option>
...
<script>
// Validasi NIK dan Nomor KK hanya angka
...
document.getElementById('formTambahPenduduk').addEventListener('submit', function(e) {
    if(!confirm('Apakah Anda yakin data yang diisi sudah benar dan ingin menyimpan data penduduk ini?')) {
        e.preventDefault();
        return false;
    }
});
