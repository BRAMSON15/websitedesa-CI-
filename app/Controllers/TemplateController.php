...
class TemplateController extends BaseController
{
    ...
    public function update($letterType)
    {
        $auth = $this->checkAuth();
        if($auth) return $auth;

        $letterType = urldecode($letterType);
        ...
        if(!$content) {
            return redirect()->back()->with('error', 'Isi template tidak boleh kosong');
        }

        // Update template in config
        $this->templateConfig->updateTemplate($letterType, $content);

        return redirect()->to('/template')->with('success', 'Template berhasil diperbarui');
    }
}